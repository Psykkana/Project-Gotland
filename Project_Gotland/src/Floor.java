...
public class Floor {
    ...
    // Some constants for the initialization
    ...
    public void initializeMap(int floorNum) {
        // Fill 22x22 grid
        for (int y = 0; y < yAxis; y++) {   // for rows
            for (int x = 0; x < xAxis; x++) {   //  for columns
                if (x == TOP_ROW ||          
                    x == BOT_ROW ||   
                    y == LEFT_COL ||           
                    y == RIGHT_ROW) {
        ...
        if (floorNum == 1) {
            setupGroundFloor();
        } // else if setupSecondFLoor when needed


    }

    // Constructs the Ground Floor
    private void setupGroundFloor() {
    ...
    // put a shopper here in the parameters when u get the final class
